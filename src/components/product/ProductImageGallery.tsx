...
const ProductImageGallery = ({ images: propImages }: ProductImageGalleryProps) => {
  const { productId } = useParams();
  const { data: product } = useProduct(productId || '');

  // Use prop images, or product images from Sanity, or placeholder
  const productImages = propImages ||
    (product ? [product.image, product.altImage].filter(Boolean) : [placeholderImage]);
        ...
        <div className="space-y-4">
          {productImages.map((image, index) => (
            ...
            >
              <img
                src={image}
                alt={`Surgical instrument view ${index + 1}`}
        ...
        <div className="relative">
          ...
          >
            <img
              ...
              alt={`Surgical instrument view ${currentImageIndex + 1}`}
